...
The Benkas PWA is a mobile app served by your Frappe/ERPNext site at
**`https://yoursite.com/benkas`**. Users never see the ERPNext desk —
...
```bash
cd ~/frappe-bench
bench get-app https://github.com/jajulasandeep583/benkas.git
bench --site yoursite.com install-app benkas
bench --site yoursite.com migrate
bench build --app benkas        # links assets — REQUIRED on production (nginx)
bench --site yoursite.com clear-cache
bench restart
# verify everything:
bench --site yoursite.com execute benkas.install.doctor
```
...
- Installed ERPNext later? Re-seed any newly possible tiles:
  `bench --site yoursite.com execute benkas.install.seed_default_modules`
...
Just open `https://yoursite.com/benkas` in the phone browser — a blue
...
> tiny service worker served at `/benkas-sw.js` (network-first, so it never
...
Tiles are rows in the **"Benkas PWA Config"** single doctype. Two ways to
edit it:

**A. In the app (System Manager):** Profile → **⚙ App Settings** — branding,
tiles, and bottom-nav pins.

**B. On the desk (full control):** open `/app/benkas-pwa-config` and add
...
(Edit `_DEFAULT_FIELDS` in `benkas/api/data.py` to change the built-ins.)
...
`benkas/api/approvals.py` (one line per doctype). Doctypes not in the
...
- Deep links: `/benkas/m/approvals?open=Sales Invoice|ACC-SINV-0001`
...
1. Desk → **Benkas PWA Config** (or in-app ⚙ App Settings).
...
(Edit `_DEFAULT_REPORT_COLUMNS` in `benkas/api/reports.py` to change.)
...
- Doctypes with required line items that aren't supported yet show a friendly
  ...
  in `benkas/api/data.py` (one small spec — see Material Request).
...
Subscriptions live in the *Benkas Push Subscription* doctype; expired
...
benkas.install.seed_default_notifications` (idempotent).
...
- Delivery runs through the background workers + scheduler —
  `benkas.install.doctor` checks they're alive.
...
```bash
cd ~/frappe-bench/apps/benkas && git pull
cd ~/frappe-bench
bench --site yoursite.com migrate
bench build --app benkas
bench --site yoursite.com clear-cache
bench restart
```
...
`bench --site <site> execute benkas.install.doctor` → must end `0 failed`.
...
| Symptom | Fix |
|---|---|
| **App blank / not loading on a production site** | `bench build --app benkas && bench restart` — production nginx serves `sites/assets`, which is created by bench build. Then run the doctor (below) |
...
| Empty home grid on a fresh site | `bench --site <s> execute benkas.install.seed_default_modules` |
...
```bash
bench --site yoursite.com execute benkas.install.diagnose
```
...
Open **`https://yoursite.com/benkas_status`** in any browser while
