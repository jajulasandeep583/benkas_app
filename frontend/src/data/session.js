...
// Reactive auth + config state for the Benkas PWA

import { reactive } from "vue";

// ── Read boot data injected by www/benkas.html Jinja template ───────────
...
export async function loadConfig(force = false) {
  ...
  try {
    const r = await apiFetch("/api/method/benkas.api.pwa.get_config");
...
export async function hardRefresh() {
  ...
  window.location.replace(`/benkas/home?r=${Date.now()}`);
}
