...
// Thin client for the generic doctype data API (benkas.api.data.*)
...
export const getView = (doctype, label, fields, filters) =>
  call("benkas.api.data.get_view", { doctype, label, fields, filters });

export const getDashboard = (target) =>
  call("benkas.api.data.get_dashboard", { target });

export const getCreateMeta = (doctype) =>
  call("benkas.api.data.get_create_meta", { doctype });

export const searchLink = (doctype, txt) =>
  call("benkas.api.data.search_link", { doctype, txt });

export const createDoc = (doctype, values) =>
  callPost("benkas.api.data.create_doc", { doctype, values: JSON.stringify(values) });

export const getList = (doctype, { search, start, page_length, fields, filters } = {}) =>
  call("benkas.api.data.get_list", { doctype, search, start, page_length, fields, filters });

export const getDoc = (doctype, name, fields) =>
  call("benkas.api.data.get_doc", { doctype, name, fields });
