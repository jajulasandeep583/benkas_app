  ...
  <ion-page>
    <ion-content :fullscreen="true">
      <div class="login-root">
        ...
        <p class="login-footer">Benkas &middot; Powered by Frappe v16</p>
      </div>
    </ion-content>
  </ion-page>
</template>
...
const brandName  = ref("Benkas");
const brandColor = ref("#6366f1");

onMounted(async () => {
  // Fetch public branding — no auth needed
  try {
    const r = await fetch("/api/method/benkas.api.pwa.get_public_config", {
      credentials: "include",
    });
    const d = await r.json();
    if (d.message) {
      brandName.value  = d.message.app_name    || "Benkas";
...
async function submit() {
  ...
  try {
    ...
    const dest = route.query.r || "/benkas/home";
    router.replace(dest);
  } catch (e) {
