      ...
      <ion-toolbar>
        ...
        <div slot="end" style="padding-right:14px;display:flex;align-items:center;gap:10px;">
          ...
          <button
            ...
            @click="router.push('/benkas/notifications')"
          >
            🔔
          ...
          <div
            ...
            @click="router.push('/benkas/profile')"
            role="button"
            :aria-label="`${session.fullname || session.user} — go to profile`"
            tabindex="0"
            @keyup.enter="router.push('/benkas/profile')"
          >{{ userInitial }}</div>
        </div>
      </ion-toolbar>
    </ion-header>
    ...
    <ion-content :fullscreen="true">
      ...
      <div v-if="appConfig.show_attendance" class="ci-wrap">
        <div class="ci-card" :class="{ 'ci-active': checkin.checked_in }">
          ...
          <button class="ci-history-link" @click="router.push('/benkas/checkin')">
...
function openModule(mod) {
  ...
  router.push(`/benkas/module/${encodeURIComponent(mod.name)}`);
}
function openConfig() {
  router.push("/benkas/settings");
}
