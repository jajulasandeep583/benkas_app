    ...
    <ion-header :translucent="true">
      <ion-toolbar>
        <ion-buttons slot="start">
          <ion-back-button default-href="/benkas/profile" text="Back" />
        </ion-buttons>
    ...
    <ion-content :fullscreen="true">
      ...
      <template v-else>
        ...
        <div class="st-nav" role="button" tabindex="0"
             @click="$router.push('/benkas/notify-settings')"
             @keyup.enter="$router.push('/benkas/notify-settings')">
...
async function load() {
  ...
  try {
    const r = await apiFetch("/api/method/benkas.api.admin.get_admin_config");
...
async function save() {
  ...
  try {
    ...
    const r = await apiFetch("/api/method/benkas.api.admin.save_admin_config", {
      method: "POST",
