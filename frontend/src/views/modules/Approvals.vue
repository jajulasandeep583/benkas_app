...
async function load() {
  ...
  error.value = null;
  try {
    const r = await apiFetch("/api/method/benkas.api.approvals.get_pending");
...
async function openDoc(it) {
  ...
  try {
    ...
    const r = await apiFetch(`/api/method/benkas.api.approvals.get_preview?${q}`);
    if (!r.ok) throw new Error(`HTTP ${r.status}`);
...
async function apply(a) {
  ...
  actionError.value = "";
  try {
    const r = await apiFetch("/api/method/benkas.api.approvals.take_action", {
      method: "POST",
      body: JSON.stringify({
