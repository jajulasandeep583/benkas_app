  ...
  <ion-page>
    <ion-tabs>
      ...
      <ion-tab-bar slot="bottom">
        <ion-tab-button tab="home" href="/benkas/home">
        ...
        <ion-tab-button
          ...
          :key="m.name"
          :tab="`mod-${m.name}`"
          :href="`/benkas/m/${encodeURIComponent(m.name)}`"
        >
          <span class="nav-emoji" aria-hidden="true">{{ m.nav_icon || m.icon || "⊞" }}</span>
          <ion-label>{{ m.nav_label || m.label }}</ion-label>
        </ion-tab-button>

        <ion-tab-button v-if="appConfig.show_attendance" tab="checkin" href="/benkas/checkin">
          <ion-icon :icon="timeOutline" aria-hidden="true" />
          <ion-label>Attendance</ion-label>
        </ion-tab-button>
        <ion-tab-button tab="profile" href="/benkas/profile">
