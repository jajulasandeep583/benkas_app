...
def extend_boot(bootinfo):
    ...
    try:
        ...
        bootinfo.mt_config = {
            "app_name":      cfg.app_name      or "Benkas",
...
def get_pwa_boot_context():
    """
    Jinja method — called from www/benkas.html template.
    ...
    if frappe.session.user != "Guest":
        try:
            cfg = frappe.get_cached_doc("Benkas PWA Config")
            out["app_name"]      = cfg.app_name      or "Benkas"
...
def _build_version():
    ...
    try:
        return int(os.path.getmtime(frappe.get_app_path("benkas", "public", "frontend", "index.js")))
    except Exception:
        return 1
...
def get_public_config():
    ...
    try:
        ...
        return {"app_name": "Benkas", "theme_color": "#6366f1"}
