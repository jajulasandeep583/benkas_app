...
Flow:
  ...
  - Tapping the notification deep-links into the app
    (/benkas/m/approvals?open=Doctype|Name).
...
_VAPID_SUB = "mailto:aibenkas@gmail.com"
...
def subscribe(subscription, user_agent=None):
    ...
    existing = frappe.get_all(
        "Benkas Push Subscription",
        filters={"endpoint": endpoint}, pluck="name", limit_page_length=1,
    )
    if existing:
        doc = frappe.get_doc("Benkas Push Subscription", existing[0])
    else:
        doc = frappe.new_doc("Benkas Push Subscription")
...
def unsubscribe(endpoint):
    for name in frappe.get_all(
        ...
        frappe.delete_doc("Benkas Push Subscription", name,
...
def get_status():
    ...
    n = frappe.db.count("Benkas Push Subscription", {"user": frappe.session.user})
    ...
    for s in subs:
        try:
            ...
            if code in (404, 410):  # subscription expired / revoked
                frappe.delete_doc("Benkas Push Subscription", s.name,
                                  ignore_permissions=True, force=True)
            else:
                frappe.log_error(f"webpush failed ({code}): {e}", "Benkas Push")
        except Exception:
            frappe.log_error(frappe.get_traceback(), "Benkas Push")
    frappe.db.commit()


def _enqueue_send(users, title, body, url=None, tag=None):
    frappe.enqueue(
        "benkas.api.push.send_to_users",
    ...
    try:
        for u in users:
            try:
                ...
                frappe.log_error(frappe.get_traceback(), "Benkas approval log")
    finally:
...
def notify_workflow_action(doc, method=None):
    ...
    try:
        ...
        _enqueue_send(
            ...
            url=f"/benkas/m/approvals?open={ref_dt}|{ref_name}",
            tag=f"wf:{ref_dt}:{ref_name}",
        )
    except Exception:
        frappe.log_error(frappe.get_traceback(), "Benkas Push (workflow)")
...
def notify_notification_log(doc, method=None):
    ...
    try:
        ...
        _enqueue_send(
            ...
            title=subject[:120],
            body=body,
            url="/benkas/notifications",
            tag=f"nl:{doc.name}",
        )
    except Exception:
        frappe.log_error(frappe.get_traceback(), "Benkas Push (notification)")
