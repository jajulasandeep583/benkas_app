...
# License: GPL-3.0
"""
/benkas_status — self-service diagnostics page (System Manager only).
...
def get_context(context):
    if frappe.session.user == "Guest":
        frappe.local.flags.redirect_location = "/login?redirect-to=/benkas_status"
        raise frappe.Redirect
    if "System Manager" not in frappe.get_roles():
        frappe.throw("Only System Managers can view this page", frappe.PermissionError)

    from benkas.install import app_version_info, collect_doctor, run_feature_tests
