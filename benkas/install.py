...
def after_install():
    """Runs after `bench --site <site> install-app benkas`.
    Auto-configures the PWA so a fresh site is usable immediately."""
    try:
        seed_number_cards()
    except Exception:
        frappe.log_error(frappe.get_traceback(), "benkas: seed_number_cards failed")

    try:
        seed_default_modules()
    except Exception:
        frappe.log_error(frappe.get_traceback(), "benkas: seed_default_modules failed")

    try:
        seed_default_notifications()
    except Exception:
        frappe.log_error(frappe.get_traceback(), "benkas: seed_default_notifications failed")

    try:
        from benkas.setup.web_pages import create_pages
        create_pages()  # creates About/Notices web pages + links them as modules
    except Exception:
        frappe.log_error(frappe.get_traceback(), "benkas: create_pages failed")

    frappe.msgprint(
        "Benkas PWA installed and configured. Open /benkas on your phone, "
        "or manage tiles at /app/benkas-pwa-config.",
        title="Benkas PWA Ready",
        indicator="green",
    )
...
def after_migrate():
    ...
    try:
        ...
        frappe.log_error(frappe.get_traceback(), "benkas: ensure_native_modules failed")
...
def seed_number_cards():
    ...
    for label, doctype, color in DEFAULT_CARDS:
        ...
        try:
            ...
            frappe.log_error(frappe.get_traceback(), "benkas: number card seed")
    if made:
        frappe.db.commit()
...
def doctor():
    """Production health check. Run on any site:
        bench --site <site> execute benkas.install.doctor
    Prints PASS/FAIL per check with the exact fix for each failure."""
    print(f"benkas doctor — site: {frappe.local.site}\n")
...
def collect_doctor():
    ...
    app_index = frappe.get_app_path("benkas", "public", "frontend", "index.js")
    check("Built frontend bundle exists (app)", os.path.exists(app_index),
          "git pull in apps/benkas (the build is committed); never delete public/frontend")
    ...
    assets_index = os.path.join(frappe.utils.get_bench_path(), "sites", "assets",
                                "benkas", "frontend", "index.js")
    check("Assets linked under sites/assets (served by nginx)", os.path.exists(assets_index),
          "run: bench build --app benkas   (creates the assets symlink) then bench restart")
    ...
    if os.path.exists(app_index) and os.path.exists(assets_index):
        ...
        check("Served bundle matches the app's build (not stale)", same,
              "run: bench build --app benkas && bench --site {} clear-cache".format(frappe.local.site))

    # 3. app installed on this site
    installed = "benkas" in frappe.get_installed_apps()
    check("App installed on this site", installed,
          "bench --site {} install-app benkas".format(frappe.local.site))
    ...
    try:
        ...
        check("PWA Config has tiles", bool(rows),
              "bench --site {} execute benkas.install.seed_default_modules".format(frappe.local.site))
        enabled = [r for r in rows if r.is_enabled]
        check("At least one tile is Enabled", bool(enabled),
              "tick 'Enabled' on tiles in /app/benkas-pwa-config")
        for r in enabled:
...
def app_version_info():
    ...
    try:
        app_dir = os.path.dirname(frappe.get_app_path("benkas"))
        out = subprocess.run(
            ["git", "-C", app_dir, "log", "-1", "--format=%h %cd %s", "--date=short"],
            capture_output=True, text=True, timeout=10,
        )
        info["benkas_commit"] = (out.stdout or out.stderr or "").strip()[:120] or "unknown"
    except Exception:
        info["benkas_commit"] = "unknown"
    return info
...
def run_feature_tests():
    ...
    def t_config():
        from benkas.api.pwa import get_config
        c = get_config()
    ...
    for row in rows:
        if row.module_type in ("doc_list", "doctype", "list_view"):
            ...
            def t_report(r=row.report_name, f=row.get("report_filters")):
                from benkas.api import reports
...
def diagnose():
    """EVERYTHING in one shot — run this when 'nothing works' on a site:
        bench --site <site> execute benkas.install.diagnose
...
def seed_default_notifications():
    """Idempotently create the default Notification rules. Re-runnable:
        bench --site <site> execute benkas.install.seed_default_notifications
    ...
    for subject, doctype, event, value_changed, rfield, rrole, message in DEFAULT_NOTIFICATIONS:
        ...
        try:
            ...
            frappe.log_error(frappe.get_traceback(), "benkas: seed_default_notifications")
    if made:
        frappe.db.commit()
