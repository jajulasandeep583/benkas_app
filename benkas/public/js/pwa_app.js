...
(function () {
  ...
  function boot() {
    initTheme();
    api("benkas.api.pwa_api.get_pwa_config", {}, "GET").then(function (c) {
      state.config = c;
      state.modules = c.modules || [];
      applyTheme(c);
      $("hTitle").textContent = c.app_name || "Benkas";
      if (c.logo) {
  ...
  function renderHome() {
    ...
    if (!state.modules.length) {
      scr.appendChild(emptyState("⊞", "No tiles yet", "Add modules in Benkas PWA Config.", null, null));
  ...
  function renderList(container, doctype, m) {
    ...
    function load(reset) {
      return api("benkas.api.pwa_api.get_list_data", {
  ...
  function renderDetail(container, doctype, name) {
    ...
    scr.appendChild(skeletonList(scr));
    api("benkas.api.pwa_api.get_doc_data", { doctype: doctype, name: name }, "GET").then(function (d) {
  ...
  function renderCardBig(container, cardName) {
    container.innerHTML = "";
    var scr = el("div", "screen"); container.appendChild(scr);
    api("benkas.api.pwa_api.get_number_card_value", { number_card_name: cardName }, "GET").then(function (c) {
  ...
  function renderDashboard(container, names) {
    ...
    var p = names && names.length
      ? Promise.all(names.map(function (n) { return api("benkas.api.pwa_api.get_number_card_value", { number_card_name: n }, "GET").catch(function () { return null; }); })).then(function (a) { return { cards: a.filter(Boolean) }; })
      : api("benkas.api.data.get_dashboard", {}, "GET");
    p.then(function (d) {
      var cards = d.cards || [];
