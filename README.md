...
This app:  benkas/www/benkas.html → yoursite.com/benkas
```

Frappe automatically serves any file in `www/` as a URL. That's it.  
`website_route_rules` in `hooks.py` makes sub-paths like `/benkas/home` also work.
...
cd ~/frappe-bench
bench get-app https://github.com/benkas/benkas

# 2 — Install on your site
bench --site yoursite.localhost install-app benkas

# 3 — Run migrations (creates DocTypes in MariaDB)
bench --site yoursite.localhost migrate

# 4 — Build frontend assets
cd apps/benkas/frontend
yarn install
yarn build
...
# 6 — Open the PWA
# Desktop: https://yoursite.com/benkas
...
# Open PWA at:
# http://yoursite.localhost:8080/benkas
```
...
2. Search **Benkas PWA Config** (or go to `/app/benkas-pwa-config`)
3. Set branding:
...
- **Android Chrome**: visit `/benkas` → tap menu → "Add to Home Screen"
...
```
benkas/                         ← Frappe app root
├── pyproject.toml                   ← v16 packaging config
├── LICENSE                          ← GPL-3.0
├── MANIFEST.in
├── .gitignore
│
├── benkas/                     ← Python package
...
│   │
│   ├── doctype/
│   │   ├── benkas_pwa_config/  ← Single DocType (global config)
│   │   └── benkas_pwa_module/  ← Child table (each module row)
│   │
│   ├── public/
...
│   │
│   └── www/
│       ├── benkas.py           ← auth guard + context
│       └── benkas.html         ← Jinja entry → boots Vue SPA
│
...
Copyright © 2024 Benkas. All rights reserved.
